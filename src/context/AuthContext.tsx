...
import { doc, getDoc, setDoc, updateDoc, deleteDoc, collection, getDocs, query, where, increment, addDoc, orderBy } from "firebase/firestore";
import { auth, db } from "../../firebaseConfig"; // Adjusted import path assuming src structure
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const requestPayout = async (amount: number, phoneNumber: string) => {
      ...
      if (amount > (userProfile.referralEarnings || 0)) throw new Error("Insufficient balance");

      try {
          const userRef = doc(db, "profiles", user.uid);
          await updateDoc(userRef, { referralEarnings: increment(-amount) });
  ...
  const processPayout = async (payoutId: string, action: 'paid' | 'rejected') => {
      ...
      try {
          ...
          if (action === 'paid') {
              ...
              await updateDoc(payoutRef, { status: 'rejected' });
              await updateDoc(doc(db, "profiles", payoutData.userId), {
