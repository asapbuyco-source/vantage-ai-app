...
import { HashRouter as BrowserRouter, Routes, Route, useNavigate, useLocation } from 'react-router-dom';
...
const LandingPage = !IS_NATIVE ? lazy(() => import('./pages/LandingPage').then(m => ({ default: m.LandingPage }))) : null;
const BlogIndex = !IS_NATIVE ? lazy(() => import('./pages/BlogIndex').then(m => ({ default: m.BlogIndex }))) : null;
const BlogPost = !IS_NATIVE ? lazy(() => import('./pages/BlogPost').then(m => ({ default: m.BlogPost }))) : null;
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => { if (event.data?.type === 'SW_UPDATED') setShowUpdateBanner(true); };
    navigator.serviceWorker.addEventListener('message', onMessage);
    return () => navigator.serviceWorker.removeEventListener('message', onMessage);
  }, []);
  useEffect(() => { if (user && !localStorage.getItem('vantage_onboarded')) setShowOnboarding(true); }, [user]);
  useEffect(() => {
  ...
  if (authLoading) return <div className="min-h-screen flex flex-col items-center justify-center bg-vantage-bg"><Loader2 className="animate-spin text-vantage-cyan mb-4" size={40} /><p className="text-gray-500 text-sm animate-pulse">Loading...</p></div>;

  if (!user) {
    if (IS_NATIVE) return <div className="min-h-screen font-sans"><main className="container mx-auto max-w-md px-4 pt-6 min-h-screen"><Profile initialMode="login" onBack={() => {}} /></main></div>;
    return (
      <Routes>
        ...
        <Route path="*" element={<div className="min-h-screen selection:bg-vantage-cyan/30 font-sans"><div className="fixed inset-0 pointer-events-none z-0"><div className={`absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-4xl h-64 blur-[100px] rounded-full mix-blend-screen ${theme === 'dark' ? 'bg-vantage-cyan/5' : 'bg-blue-200/40'}`} /></div><main className="relative z-10 container mx-auto max-w-md md:max-w-6xl px-4 pt-6 min-h-screen"><AnimatePresence mode="wait">{authView === 'landing' ? <MotionDiv key="landing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, x: -20 }}><Suspense fallback={<div className="min-h-screen flex items-center justify-center"><Loader2 className="animate-spin" size={32} /></div>}>{LandingPage && <LandingPage onGetStarted={() => setAuthView('signup')} onLogin={() => setAuthView('login')} onShowStats={() => setAuthView('stats')} />}</Suspense></MotionDiv> : authView === 'stats' ? <MotionDiv key="stats" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}><div className="flex flex-col min-h-screen"><div className="flex items-center gap-2 py-4 mb-2"><button onClick={() => setAuthView('landing')} className="p-2 bg-white/5 rounded-lg text-gray-500 hover:text-vantage-cyan"><X size={20} /></button><span className="text-sm font-bold uppercase tracking-widest text-gray-400">Back</span></div><PublicStats /><div className="mt-auto py-8"><button onClick={() => setAuthView('signup')} className="w-full py-4 bg-white text-slate-900 font-bold rounded-xl shadow-lg">Get Started Free</button></div></div></MotionDiv> : <MotionDiv key="auth" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}><div className="max-w-md mx-auto"><Profile initialMode={authView === 'login' ? 'login' : 'signup'} onBack={() => setAuthView('landing')} /></div></MotionDiv>}</AnimatePresence></main></div>} />
      </Routes>);
  }
      ...
      <Route path="*" element={
        <div className="min-h-screen selection:bg-vantage-cyan/30 font-sans md:flex">
          ...
          {!IS_NATIVE && (
            <AnimatePresence>
              {showUpdateBanner && (
                <MotionDiv initial={{ y: -60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -60, opacity: 0 }}
                  ...
                  <div className="flex items-center gap-2"><RefreshCw size={15} className="animate-spin" /><span>New version available</span></div>
          ...
          <main className="relative z-10 w-full mx-auto max-w-md md:max-w-7xl md:ml-64 px-4 pt-6 min-h-screen pb-24 md:pb-6">
            <AnimatePresence mode="wait">
              <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><Loader2 className="animate-spin text-vantage-cyan" size={40} /></div>}>
                <ErrorBoundary>
                  <Routes>
          <Route path="/" element={<Home />} /><Route path="/free" element={<Home />} />
        ...
        </Routes></ErrorBoundary></Suspense></AnimatePresence></main>
        <BottomNav /><AnimatePresence>{showOnboarding ? <Onboarding onComplete={handleOnboardingComplete} /> : null}</AnimatePresence><BetSlip /><ToastContainer />
      </div>} />
    </Routes>);
}

export default function App() {
  return <BrowserRouter><AppProvider><AuthProvider><DataProvider><AppContent /></DataProvider></AuthProvider></AppProvider></BrowserRouter>;
}
