...
function AppContent() {
  ...
  // Android back button - navigate history instead of closing
  useEffect(() => {
    if (!IS_NATIVE) return;
  ...
  // RevenueCat init + link to Firebase UID
  useEffect(() => {
    if (!IS_NATIVE) return;
    const setup = async () => {
      try {
                ...
                {authView === 'landing' ? (
                  ...
                  <MotionDiv key="stats" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
                    <div className="flex flex-col min-h-screen">
                      ...
                      <PublicStats /><div className="mt-auto py-8"><button onClick={() => setAuthView('signup')} className="w-full py-4 bg-white text-slate-900 font-bold rounded-xl shadow-lg">Get Started Free</button></div>
                    </div>
                  </MotionDiv>
                ) : (
